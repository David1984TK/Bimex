...
Check `PINATA_API_KEY` and `PINATA_SECRET` are set in the `bimex-indexer` environment (server-side only, never `VITE_` prefixed — see `POST /ipfs-upload` in `docs/api.md`).
