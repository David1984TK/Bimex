...
The API protects public read endpoints and long-lived SSE connections with a generic limiter in `bimex-indexer/rateLimiter.js`.
...
| `/sse` | `5` simultaneous connections | Client IP | Connections are released when the HTTP request closes. |
...
| `RATE_LIMIT_SSE_SUPABASE_TTL_SECONDS` | `90` | TTL used to auto-expire orphaned shared SSE slots if a process dies. |
...
| `RATE_LIMIT_WHITELIST_IPS` | empty | Comma-separated exact IPs or IPv4 CIDRs that bypass IP-based public/SSE limits. Useful for trusted internal infrastructure such as Vercel rewrites. |
| `RATE_LIMIT_TRUSTED_IPS` / `INTERNAL_IP_WHITELIST` / `FRONTEND_VERCEL_IP_WHITELIST` | empty | Additional whitelist aliases. |

The API reads the client IP from `X-Forwarded-For`, then `X-Real-IP`, then the socket remote address. Deploy behind a trusted reverse proxy so these headers cannot be spoofed by direct internet clients.
...
- `consume_rate_limit_bucket(...)`: atomic RPC used by the API for fixed-window limits.
- `acquire_sse_connection(...)`, `heartbeat_sse_connection(...)`, `release_sse_connection(...)`: RPCs used for shared SSE connection limits.
- `cleanup_rate_limit_data(...)`: optional cleanup helper.

If the Supabase RPC/tables are not available, the API logs a warning and temporarily falls back to in-memory buckets/connection counters so local development and migrations remain non-breaking.

Blocked requests are always logged with `console.warn`; when Supabase is configured and the log table exists, blocked IPs are also inserted into `rate_limit_blocked_events` for later review.

## Validation

Run the deterministic rate-limit tests:
...
The test suite simulates burst/load behavior for the 60 req/min public endpoint policy, verifies RFC/legacy headers, checks SSE concurrent connection caps, and validates IP whitelist matching.
